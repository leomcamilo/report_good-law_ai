...
arquivo_dados = 'csv_longo/projetos_lei_do_bem_2023_DETALHADO_LINHA_UNICA.csv'
...
colunas_analise = (colunas_identificacao + colunas_projeto + 
                   colunas_analista + colunas_parecerista + colunas_valores)
...
print(f"├── Analista: {len([c for c in colunas_analista if c in colunas_existentes])}")
print(f"├── Parecerista: {len([c for c in colunas_parecerista if c in colunas_existentes])}")
...
```
    ...
    ├── Palavras médias (original): 282.5
    ├── Palavras médias (limpo): 154.8
    └── Redução média: 44.8%

    Processando: Elemento Tecnológico
    ├── Palavras médias (original): 326.2
    ├── Palavras médias (limpo): 179.0
    └── Redução média: 44.9%

    Processando: Desafio Tecnológico
    ├── Palavras médias (original): 307.0
    ├── Palavras médias (limpo): 168.5
    └── Redução média: 44.8%

    Processando: Metodologia Utilizada
    ├── Palavras médias (original): 297.4
    ├── Palavras médias (limpo): 165.2
    └── Redução média: 43.5%
...
![](analise_analistas_parecerista_files/figure-markdown_strict/cell-4-output-3.png)
    ...
    Limpo: ano-base 2021, jardinox investiu pesquisa, procura soluções setor transporte, aplicado principalmente produtos viabilizassem fluxo logístico abastecim...
    Redução: 45.7%
